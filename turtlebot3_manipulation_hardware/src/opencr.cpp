...
std::array<double, 2> OpenCR::get_wheel_efforts()
{
  std::array<double, 2> torques = {0.0, 0.0};

  std::array<int32_t, 2> currents = {
    get_data<int32_t>(
      opencr_control_table.present_current_left.address,
      opencr_control_table.present_current_left.length),
    get_data<int32_t>(
      opencr_control_table.present_current_right.address,
      opencr_control_table.present_current_right.length)
  };
...
std::array<double, 4> OpenCR::get_joint_efforts()
{
  std::array<double, 4> torques = {0.0, 0.0, 0.0, 0.0};

  // unit is milliAmpare * 2.69
  std::array<int16_t, 4> currents = {
    get_data<int16_t>(
      opencr_control_table.present_current_joint_1.address,
      opencr_control_table.present_current_joint_1.length),
    get_data<int16_t>(
      opencr_control_table.present_current_joint_2.address,
      opencr_control_table.present_current_joint_2.length),
    get_data<int16_t>(
      opencr_control_table.present_current_joint_3.address,
      opencr_control_table.present_current_joint_3.length),
    get_data<int16_t>(
      opencr_control_table.present_current_joint_4.address,
      opencr_control_table.present_current_joint_4.length),
  };

  // do current<->torque calculation
  // torque = kt * current     where kt is motor torque coef.
  //        = 1/kv * current   where kv is relationship of peak voltage and rotation speed
...
double OpenCR::get_gripper_effort()
{
  double torque = 0.0;

  int16_t current = get_data<int32_t>(
